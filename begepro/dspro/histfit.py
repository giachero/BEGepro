...
from begepro.dspro import functions
...
class HistogramFitter (object):
    ...
    def __init__(self, countsperbin, binedges, bin_spacing = 1):
        ...
        self.__pars = dict()
        self.__bin_spacing = bin_spacing
        return
    ...
    def net_counts(self):
        if 'ntail' in self.__pars:
            counts = (self.__pars['ngaus']['opt_value'] + self.__pars['ntail']['opt_value']) / self.__bin_spacing
            error = np.sqrt(self.__pars['ngaus']['err_opt_value']**2 + self.__pars['ntail']['err_opt_value']**2) / self.__bin_spacing
            return counts, error
        else:
            return self.__pars['ngaus']['opt_value'] / self.__bin_spacing, self.__pars['ngaus']['err_opt_value'] / self.__bin_spacing
    ...
    def plot_fit(self, xlim=None, nop=1000, plot = None, color = None, lw = None):
        if xlim is None:
            xlim = self.__xlim
        popt = [self.__pars[par]['opt_value'] for par in self.__pars]
        xfnc = np.linspace(xlim[0], xlim[1], num=nop)
        if plot is None:
            plt.plot(xfnc, self.__f(xfnc, *popt), label='fit', linewidth=lw)
            plt.legend()
        else:
            plot.plot(xfnc, self.__f(xfnc, *popt), label='fit', linewidth=lw)
        return
